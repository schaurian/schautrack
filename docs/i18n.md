...
> **Aside (same issue bullet):** the bullet also flagged dead release tooling.
> `scripts/bump-version.sh` has since been removed (it duplicated `scripts/next-version.sh`'s
> CI version logic with a *different*, scope-aware regex, which is exactly what let a
> scoped `feat(x):` commit silently mis-version two releases before anyone noticed — fixed
> as its own change, unrelated to i18n). `scripts/generate-changelog.sh` is still dead and
> referenced by nothing; that one remains tracked here only so it is not lost, and should
> be handled as its own cleanup, not folded into this work.
...
- **Not** bundling the dead `scripts/generate-changelog.sh` cleanup into this work —
  `scripts/bump-version.sh` was already removed (see §1); `generate-changelog.sh` is
  noted there too and handled separately.
