...
| `RATE_LIMIT_BARCODE` | `30` | Max barcode lookups per minute, applied per IP on the app's own route and per **account** on `GET /api/v1/barcode/{code}`. One value for both so the public API cannot become the cheaper path to the same third-party database. Raise it if your users hit `429` while scanning. |
