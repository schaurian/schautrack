...
//
// rawAmount comes from optionalString, so an absent or null JSON key arrives
// here as the empty string — never the "<nil>" sentinel the old
// fmt.Sprintf("%v", …) coercion produced. A JSON number >= 1e6 still arrives in
// scientific notation ("1e+06"), which ParseAmount rejects on syntax. That
// rejection is the right answer, because any such value is far past
// MaxEntryCalories, but it is worth knowing the 400 is not a range check.
//
